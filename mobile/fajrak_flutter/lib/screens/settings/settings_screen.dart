...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _exportData() async {
    ...
    final txRes = await Supabase.instance.client.from('transactions').select('*').eq('user_id', user.id).order('transaction_date', ascending: false);
    final data = txRes as List;
    if (data.isEmpty) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('لا توجد معاملات لتصديرها', style: TextStyle(fontFamily: 'Cairo'))));
    ...
    for (final tx in data) {
      ...
      buffer.writeln('${tx['transaction_date']},$type,${tx['amount']},${tx['category'] ?? ''},${tx['description'] ?? ''}');
    }

    final directory = await getTemporaryDirectory();
    final file = File('${directory.path}/fajrak_export_${DateTime.now().millisecondsSinceEpoch}.csv');
  ...
  void _shareApp() {
    const text = 'جربت فجرك؟ تطبيق مالي عربي مجاني يساعدك تتحكم في مصاريفك وتبني ثروتك 🌅\n\nhttps://fajrak.com';
    Share.share(text);
  }
      ...
      body: _loading
          ...
          : SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _accordionCard(
                  icon: '👤', title: 'الملف الشخصي', initiallyExpanded: true,
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        Container(
                          width: 50, height: 50,
                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), gradient: const LinearGradient(colors: [Color(0xFF3B7EF6), Color(0xFF8B5CF6)])),
                          child: Center(child: Text(_nameCtrl.text.isNotEmpty ? _nameCtrl.text.substring(0, _nameCtrl.text.length.clamp(0, 2)).toUpperCase() : _userEmail.substring(0, 2).toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16, fontFamily: 'Cairo'))),
                        ),
                        const SizedBox(width: 12),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(_nameCtrl.text.isNotEmpty ? _nameCtrl.text : 'اسمك', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 14, fontFamily: 'Cairo')),
                          Text(_userEmail, style: const TextStyle(color: Color(0xFF64748B), fontSize: 11, fontFamily: 'Cairo')),
                        ])),
                        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                          const Text('عضو', style: TextStyle(color: Color(0xFF64748B), fontSize: 9, fontFamily: 'Cairo')),
                          Text(_memberSince, style: const TextStyle(color: Color(0xFF94A3B8), fontWeight: FontWeight.w800, fontSize: 11, fontFamily: 'Cairo')),
                        ]),
                      ]),
                    ),
                ...
                _accordionCard(
                  icon: '💎', title: 'أصولي الشخصية', badge: 'صافي الثروة',
                  child: Column(children: [
                    Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(gradient: LinearGradient(colors: [const Color(0xFF3B7EF6).withOpacity(0.05), const Color(0xFF10B981).withOpacity(0.03)]), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFF3B7EF6).withOpacity(0.15))),
                      child: Column(children: [
                        const Text('💎 صافي ثروتك', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12, fontFamily: 'Cairo', fontWeight: FontWeight.w700)),
                        const SizedBox(height: 6),
                        Text('${netWorth.toStringAsFixed(0)} $_currency', style: TextStyle(color: netWorth >= 0 ? const Color(0xFF10B981) : const Color(0xFFEF4444), fontSize: 24, fontWeight: FontWeight.w900, fontFamily: 'Cairo')),
                        const SizedBox(height: 12),
                ...
                _accordionCard(
                  icon: '📥', title: 'تصدير البيانات',
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('قم بتصدير جميع معاملاتك إلى ملف CSV لاستخدامه في برامج مثل Excel.', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12, fontFamily: 'Cairo', height: 1.6)),
                    const SizedBox(height: 16),
                    SizedBox(width: double.infinity, child: ElevatedButton.icon(
                      onPressed: _loading ? null : _exportData,
                      icon: const Icon(Icons.file_download_outlined, size: 20),
                      label: const Text('تصدير المعاملات (CSV)', style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w900)),
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0F1629), foregroundColor: Colors.white, side: const BorderSide(color: Color(0xFF1E293B)), padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                    )),
                  ]),
                ),
                const SizedBox(height: 12),
                ...
                // Share App (Accordion)
                _accordionCard(
                  icon: '🔗', title: 'شارك فجرك مع أصدقائك',
                  child: Column(children: [
                    const Text('🌅', style: TextStyle(fontSize: 40)),
                    const SizedBox(height: 10),
                    const Text('ساعدنا في نشر الوعي المالي', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontFamily: 'Cairo', fontSize: 14)),
                    const SizedBox(height: 8),
                    const Text('شارك التطبيق مع العائلة والأصدقاء لتبدأوا رحلة الثراء سوياً.', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12, fontFamily: 'Cairo')),
                    const SizedBox(height: 20),
                    SizedBox(width: double.infinity, child: ElevatedButton(
                      onPressed: _shareApp,
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF3B7EF6), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                ...
                _accordionCard(
                  icon: '⚠️', title: 'الحساب ومنطقة الخطر',
                  child: Column(children: [
                    // Logout
                    ...
                    if (!_showDeleteConfirm) ...[
                      SizedBox(width: double.infinity, child: TextButton(
                        ...
                        style: TextButton.styleFrom(foregroundColor: const Color(0xFFEF4444).withOpacity(0.7), padding: const EdgeInsets.symmetric(vertical: 12)),
                        child: const Text('🗑️ حذف حسابي نهائياً', style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 12)),
                      )),
                    ] else ...[
                      const SizedBox(height: 8),
  ...
  Widget _accordionCard({required String icon, required String title, String? badge, required Widget child, bool initiallyExpanded = false}) => Container(
    margin: const EdgeInsets.only(bottom: 2),
    decoration: BoxDecoration(color: const Color(0xFF0F1629), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFF1E293B))),
    ...
    child: Theme(
      ...
      child: ExpansionTile(
        ...
        title: Row(children: [
          ...
          Text(title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontFamily: 'Cairo', fontSize: 13)),
          if (badge != null) ...[
            const SizedBox(width: 8),
            Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: const Color(0xFF3B7EF6).withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFF3B7EF6).withOpacity(0.2))), child: Text(badge, style: const TextStyle(color: Color(0xFF3B7EF6), fontSize: 9, fontWeight: FontWeight.w800, fontFamily: 'Cairo'))),
          ]
        ]),
