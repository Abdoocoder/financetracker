    ...
    <>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(7,11,20,0.97)', backdropFilter: 'blur(20px)', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', padding: '8px 4px 18px' }} className="mobile-nav">
        ...
        {MAIN_NAV.map(({ href, Icon, ar, en }) => {
          ...
          return (
            <Link key={href} href={href} style={{ flex: 1, textDecoration: 'none' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '4px 2px', position: 'relative' }}>
                {isActive && <div style={{ position: 'absolute', top: -1, left: '50%', transform: 'translateX(-50%)', width: 40, height: 34, borderRadius: 12, background: 'var(--accent-blue-dim)', border: '1px solid rgba(59,126,246,0.2)' }} />}
        ...
        <button onClick={() => setShowMore(true)} style={{ flex: 1, background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '4px 2px', position: 'relative' }}>
            {isMoreActive && <div style={{ position: 'absolute', top: -1, left: '50%', transform: 'translateX(-50%)', width: 40, height: 34, borderRadius: 12, background: 'var(--accent-blue-dim)', border: '1px solid rgba(59,126,246,0.2)' }} />}
            <div style={{ position: 'relative', zIndex: 1 }}>
              <MoreHorizontal size={22} color={isMoreActive ? 'var(--accent-blue-light)' : 'var(--text-muted)'} strokeWidth={isMoreActive ? 2.5 : 1.8} />
              {alertsCount > 0 && (
                <span style={{ position: 'absolute', top: -5, right: -6, minWidth: 16, height: 16, borderRadius: 8, background: 'var(--accent-red)', color: 'white', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>
              ...
              {MORE_NAV.map(({ href, Icon, ar, en }) => {
                ...
                return (
                  <Link key={href} href={href} onClick={() => setShowMore(false)} style={{ textDecoration: 'none' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 16, background: isActive ? 'var(--accent-blue-dim)' : 'var(--bg-card)', border: `1px solid ${isActive ? 'rgba(59,126,246,0.2)' : 'var(--border)'}` }}>
                      <div style={{ width: 44, height: 44, borderRadius: 13, background: isActive ? 'rgba(59,126,246,0.15)' : 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0 }}>
