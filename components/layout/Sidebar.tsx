...
export default function Sidebar({ alertsCount = 0 }: { alertsCount?: number }) {
  ...
  return (
    <>
              ...
              <Link key={item.href} href={item.href} style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 12, background: isActive ? 'var(--accent-blue-dim)' : 'transparent', border: `1px solid ${isActive ? 'rgba(59,126,246,0.2)' : 'transparent'}`, color: isActive ? 'var(--accent-blue-light)' : 'var(--text-secondary)', fontSize: 13, fontWeight: isActive ? 700 : 500, transition: 'all 0.15s', position: 'relative' }}>
                  ...
                  {item.href === '/dashboard/alerts' && alertsCount > 0 && (
                    <span style={{ minWidth: 18, height: 18, borderRadius: 9, background: 'var(--accent-red)', color: 'white', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 8px var(--accent-red-glow)' }}>{alertsCount}</span>
                  )}
                </div>
              </Link>
            )
      ...
      {/* ── Mobile Bottom Nav ── */}
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(13,18,33,0.97)', backdropFilter: 'blur(20px)', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', padding: '8px 4px 12px', gap: 2 }} className="mobile-nav">
        {NAV.map(item => {
          const isActive = item.href === '/dashboard' ? pathname === '/dashboard' : pathname.startsWith(item.href)
          return (
            <Link key={item.href} href={item.href} style={{ flex: 1, textDecoration: 'none' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '6px 2px', position: 'relative' }}>
                {isActive && <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 28, height: 28, borderRadius: 10, background: 'var(--accent-blue-dim)', border: '1px solid rgba(59,126,246,0.25)' }} />}
                <div style={{ fontSize: 16, lineHeight: 1, position: 'relative', zIndex: 1, color: isActive ? 'var(--accent-blue-light)' : 'var(--text-muted)', transition: 'color 0.15s' }}>
                  {item.icon}
                  {item.href === '/dashboard/alerts' && alertsCount > 0 && (
                ...
                <span style={{ fontSize: 9, fontWeight: isActive ? 700 : 500, color: isActive ? 'var(--accent-blue-light)' : 'var(--text-muted)', position: 'relative', zIndex: 1 }}>
                  {item.label}
                </span>
              </div>
            </Link>
        ...
        <button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')} style={{ flexShrink: 0, width: 36, height: 36, borderRadius: 10, background: 'var(--accent-blue-dim)', border: '1px solid rgba(59,126,246,0.2)', color: 'var(--accent-blue-light)', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {lang === 'ar' ? 'EN' : 'ع'}
        </button>
      </nav>
