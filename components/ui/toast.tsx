...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const styles: Record<ToastType, { bg: string; border: string; color: string }> = {
    success: { bg: 'rgba(16,185,129,0.12)', border: 'rgba(16,185,129,0.35)', color: 'var(--accent-green-light)' },
    error:   { bg: 'rgba(239,68,68,0.12)',  border: 'rgba(239,68,68,0.35)',  color: 'var(--accent-red-light)'   },
    warning: { bg: 'rgba(245,158,11,0.12)', border: 'rgba(245,158,11,0.35)', color: '#f59e0b'                   },
    info:    { bg: 'rgba(59,130,246,0.12)', border: 'rgba(59,130,246,0.35)', color: 'var(--accent-blue-light)'  },
  }
      ...
      <div style={{
        ...
        {toasts.map(t => (
          <div key={t.id}
            ...
            <span style={{ fontSize: '16px', color: styles[t.type].color, opacity: 0.6, flexShrink: 0 }}>✕</span>
          </div>
        ))}
      </div>
