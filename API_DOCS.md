...
- **التسجيل:** بلا مفاتيح — `bodyBytes` + `bodySha256` فقط، لا يَرصد رؤوس المصادقة أبداً؛ النواتج التفصيلية (نص الطلب الممرَّر وتوقيت إتلاف المفتاح) معطّلة افتراضياً وتُفعَّل بـ `BYOK_PROXY_DEBUG=true`.
