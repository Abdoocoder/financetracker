...
// ─── Step 1 ───────────────────────────────────────────────────────────────────
function Step1({ profile, setProfile, onNext, loading }: {
  profile: Profile
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        <div style={{
          ...
          background: 'var(--accent-blue-dim)', border: '1px solid rgba(59,126,246,0.25)',
      ...
      <Field label="راتبك الشهري (اختياري)">
        <StyledInput
          type="number" value={profile.monthlyIncome}
      ...
      <button onClick={onNext} disabled={!profile.fullName.trim()} style={{
        width: '100%', padding: '14px', borderRadius: 14, border: 'none',
        background: 'var(--accent-blue)', color: 'white',
        fontSize: 15, fontWeight: 700,
        cursor: profile.fullName.trim() ? 'pointer' : 'not-allowed',
        fontFamily: 'inherit', marginTop: 8,
        opacity: profile.fullName.trim() ? 1 : 0.5,
        transition: 'opacity 0.15s, transform 0.15s',
      }}
        onMouseEnter={e => { if (profile.fullName.trim()) e.currentTarget.style.transform = 'scale(1.01)' }}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 28, margin: '0 auto 16px', transition: 'all 0.3s',
        }}>
          {tx.type === 'income' ? '💰' : '💸'}
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, padding: 4, borderRadius: 14, background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
        {(['income', 'expense'] as const).map(type => (
          <button key={type} onClick={() => setTx(t => ({ ...t, type, category: '' }))} style={{
            flex: 1, padding: '10px', borderRadius: 11, border: 'none',
            background: tx.type === type ? (type === 'income' ? 'var(--accent-green)' : 'var(--accent-red)') : 'transparent',
      ...
      <Field label="وصف (اختياري)">
        <StyledInput type="text" value={tx.description}
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
        ...
        <button onClick={onNext} disabled={saving || !tx.amount || !tx.category} style={{
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        <div style={{
          ...
          background: 'var(--accent-green-dim)', border: '2px solid rgba(16,185,129,0.3)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <button onClick={onGo} style={{
        ...
        fontFamily: 'inherit', transition: 'transform 0.15s', letterSpacing: '-0.01em',
      }}
...
export default function OnboardingPage() {
  ...
  const [profile, setProfile] = useState<Profile>({
    fullName: '', monthlyIncome: '', currency: 'JOD',
  })
  ...
  // ✅ جلب البيانات الموجودة من profiles تلقائياً
  useEffect(() => {
    async function loadExistingData() {
      ...
      const { data } = await supabase
        .from('profiles')
        .select('full_name, monthly_salary, currency')
        .eq('id', user.id)
        .single()
      ...
      if (data) {
        setProfile({
          ...
          monthlyIncome: data.monthly_salary?.toString() ?? '',
          currency: (data.currency as Profile['currency']) ?? 'JOD',
        })
      }
  ...
  async function handleStep1() {
    ...
    await supabase.from('profiles').upsert({
      ...
      monthly_salary: profile.monthlyIncome ? parseFloat(profile.monthlyIncome) : null,
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 420, position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{
            width: 40, height: 40, borderRadius: 12, background: 'var(--accent-blue)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: 'white', fontWeight: 900, fontSize: 18, margin: '0 auto 8px',
          }}>ف</div>
        ...
        <div style={{
          ...
          {step === 1 && (
            <Step1 profile={profile} setProfile={setProfile} onNext={handleStep1} loading={loadingProfile} />
          )}
          {step === 2 && (
            <Step2 tx={tx} setTx={setTx} onNext={handleStep2} onBack={() => setStep(1)} saving={saving} />
          )}
