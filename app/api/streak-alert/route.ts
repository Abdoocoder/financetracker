...
async function sendStreakAlerts() {
  ...
  await Promise.all(profiles.map(async (profile) => {
    ...
    if ((todayCount ?? 0) > 0) return // سجّل اليوم، لا يحتاج تحذير

    // احسب الـ streak الحالي (عدد الأيام المتواصلة قبل اليوم)
    ...
    if (!txs?.length) return // لا يوجد streak أصلاً
    ...
    // فقط من عنده streak >= 3 أيام يستحق التحذير
    if (streak < 3) return
