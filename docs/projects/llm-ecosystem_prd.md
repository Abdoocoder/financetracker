...
> **قرار معماري (AD-4): نموذج مفتاح BYOK = تغليف KEK (envelope encryption).**
> الاقتراح الأصلي في §4.3 كان متناقضاً: مفتاح `non-extractable` (لا يُصدَّر/يُغلَّف)
> **لا يمكن أن يصل إلى الخادم ليُفكَّ تشفيره** (حسب Web Crypto API، `wrapKey` يتطلب
> مفتاحاً قابلاً للتصدير). الحل المتماسك الوحيد مع التزام الوكيل الرقيق (C2/HOLD SCOPE):
...
> - **لكل طلب (فك تشفير الخادم بلا تخزين):** يشفّر العميل مفتاح المزوّد تحت مفتاح
>   تغليف مؤقت (ephemeral envelope key) ويغلّفه إلى **KEK محفوظ لدى الخادم** عبر
>   `wrapKey`، ويرسل `[ciphertext] + [مفتاح التغليف المغلَّف]` إلى `/api/byok/proxy`.
>   يفتح الخادم الـ KEK (`unwrapKey` — سر بيئي/secret-manager، **ليس** DB/قرص)،
>   يفك تشفير مفتاح المزوّد في الذاكرة، يبدّل رأس المصادقة، يبثّ، ثم **يُتلف**
>   المفتاح والتغليف في `finally`.
> - **AD-3 صحيح:** الـ KEK سر لإدارة المفاتيح وليس مفتاح LLM مخزّن للمستخدم؛
>   مفاتيح المستخدم لا تمسّ DB أو القرص أبداً.
> - **دورة حياة الـ KEK:** KEK عام واحد لإصدار v1 مع **وسم key-ID على كل ciphertext**
>   لتمكين **تدوير إضافي** (يُبقى الـ KEK القديم حتى يُعاد تغليف كل الشظايا؛
>   الإبطال يعمل بعد التدوير). KEK لكل مستخدم مؤجَّل.
> المزايا: يُلبي الوكيل الملتزم (C2) وAD-3 معاً؛ نمط BYOK معياري؛ خادم عديم الحالة.
> المخاطر: تسريب الـ KEK يفكك كل المفاتيح → يُخفَّف بخطة التدوير + secret-manager.
